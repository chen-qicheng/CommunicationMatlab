function BitData = demodqam16exhaust_serial(Conste, Normalized)
% 16QAM解调，穷举法
...
for SampleIdx = 1 : SampleCnt
    ...
    if length(BinaryNumber) < 4
        BitDataMatrix(:, SampleIdx) = [zeros(4 - length(BinaryNumber), 1); BinaryNumber];
    else
